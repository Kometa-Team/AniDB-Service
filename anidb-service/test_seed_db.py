...
async def test_main_success(test_seed_db_env, sample_xml, capsys):
    ...
    (xml_dir / "AnimeDoc_2.xml").write_text(
        sample_xml.replace('id="1"', 'id="2"'), encoding="utf-8"
    )
...
async def test_main_with_failed_files(test_seed_db_env, capsys):
    ...
    (xml_dir / "AnimeDoc_1.xml").write_text(
        "<anime id='1'><title>Valid</title></anime>", encoding="utf-8"
    )
